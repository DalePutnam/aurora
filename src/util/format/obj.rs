...
impl fmt::Display for Error
{
	...
	{
		match self {
			Self::ParseError { file_name, line_number, message } => {
...
fn parse_line(line: String, mesh: &mut Mesh) -> Result<(), String>
{
	...
	if line_parts.len() >= 1 {
		match line_parts[0] {
			...
			_ => {
				return Err(format!("\"{}\" tag not supported", line_parts[0]))
			}
		};
	}
...
{
	if parts.len() < 4 {
		return Err(String::from("Vertices with fewer than 3 coordinates are not supported"));
	} else if parts.len() > 4 {
		return Err(String::from("Vertices with more than 3 coordinates are not supported"));
	}
...
{
	if parts.len() < 4 {
		return Err(String::from("Faces with fewer than 3 vertices are not supported"));
	} else if parts.len() > 4 {
		return Err(String::from("Faces with more than 3 vertices are not supported"));
	}
...
fn parse_face_component(component: &str) -> Result<(usize, Option<usize>, Option<usize>), String>
{
	...
	if parts.len() == 1 {
		...
		Err(String::from("Face vertices cannot have more than 3 components"))
	}
}
