...
fn trace_worker(cpu: usize, stw: Matrix4<f32>, eye: Vector4<f32>,
    scene: &Scene, frame_sections: Arc<Mutex<Vec<FrameSection>>>, tx: Sender<PixelColour>) {
    loop {
...
fn trace_pixel(ray: &Ray, scene: &Scene) -> Vector3<f32> {
    match scene.check_hit(ray) {
        Some((hit, material)) => {
            material.shade_pixel(&ray, &hit, scene)
        },
        None => {
