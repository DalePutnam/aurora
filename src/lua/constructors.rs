...
use na;
use lua;
use rlua::{Context, Value, FromLua};


impl Material {
    pub fn lua_new<'lua>(lua: Context<'lua>, lua_value: (Value<'lua>, Value<'lua>, Value<'lua>)) -> rlua::Result<lua::Pointer<Material>> {
        ...
        Ok(lua::Pointer::new(Material::new(na::Vector3::from(diffuse), na::Vector3::from(specular), shininess)))
    }
}

impl NonhierSphere {
    pub fn lua_new<'lua>(lua: Context<'lua>, lua_value: (Value<'lua>, Value<'lua>, Value<'lua>)) -> rlua::Result<lua::SceneNode> {
...
impl Sphere {
    pub fn lua_new<'lua>(lua: Context<'lua>, lua_name: Value<'lua>) -> rlua::Result<lua::SceneNode> {
        let name = String::from_lua(lua_name, lua)?;
...
impl Mesh {
    pub fn lua_new<'lua>(lua: Context<'lua>, lua_value: (Value<'lua>, Value<'lua>)) -> rlua::Result<lua::SceneNode> {
...
impl Light {
    pub fn lua_new<'lua>(lua: Context<'lua>, lua_value: (Value<'lua>, Value<'lua>, Value<'lua>)) -> rlua::Result<Light> {
        ...
        Ok(Light::new(&na::Vector3::from(position), &na::Vector3::from(colour), &na::Vector3::from(falloff)))
    }
}